...
procedure delete(value K, pointer P)
	...
	// 还要考虑键不在树中的情况
	// 不仅要检测键, 还要检测值, 那么下面就不用检测了
	delete_entry(L, K, P)

procedure delete_entry(node N, value K, pointer P)  // 参数P在代码块中的唯一作用是找到兄弟
	delete(K, P) from N  // 同上, 表示动作而非调用
	// 我们想一下, 在结点中删除一个pair需要value(也就是这里的P)么? 不需要, 而且其已经在上面被检测了
	// call back, 所以其实在进入这段代码之前就已经确定结点类型了
	if (N is the root and N has only one remaining child)  // 这里默认结点类型是internal
	then make the child of N the new root of the tree and delete N
	else if (N has too few value/pointer) then begin  // 低于对应类型的下限, 需要类型
		...
		if (entries in N and N_ can fit in a single node)
			then begin  // coalesce nodes
				...
				// 这里是否可以直接交换指针? 不行, 1. 和parent的关系, next page id问题
				// 期望N_ N
				// 然后全部移动到左边
