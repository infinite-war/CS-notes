...
### 将别的项目的各个分支push到自己的repo下

在写xv6的实验时，他是每个实验一个分支，我们可以在MIT的项目（clone到本地）下，这样
```bash
git remote set-url origin git@你的
```
这样就会把对应的分支push到对应的repo中  

如果下一个实验换分支，同样，到MIT的项目（克隆到本地）下、新的分支下，然后上面的命令，就是将新的分支push到这个项目下了。